...
> 1. **본질**: 재해 복구 (DR) 아키텍처 - RTO (Recovery Time Objective), RPO (Recovery Point Objective)은(는) 소프트웨어 공학의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
- **개념**: 재해 복구(DR)는 업무 연속성 계획(BCP)의 기술적 구현으로, 데이터 손실과 시스템 downtime을 최소화하기 위한 체계적인 전략이다. DR은 화재,지진, 테러 공격, Ransomware 공격 등 예기치 않은 대규모 Incident에 대응하여, 비즈니스가 사용할 수 있는 수준의 시스템으로 얼마나 빨리 돌아갈 수 있는지를 목표로 한다.

- **필요성**: 2021년 한국모 datacenter 화재로 인해 금음카드, 한겨레 등 주요 서비스가 수일에 걸쳐 마비된 사례에서 볼 수 있듯이, DR 전략이 부재하거나불십분하면 대규모 장애발생 시 회복이 매우 어렵다. 또한 금전적 손실 외에 고객 신뢰 상실, 규제 제재 등 이다음적피해도 발생한다.

- **💡 비유**: DR 아키텍처는 <strong>'항공사 비행기의 안전설비'</strong>와 같다. 비행기에는 엔진고장 시를 위한 이중 엔진,긴급 대피용 미끄럼틀, 충돌 가능성에 대한회피 Procedures 등이 있으며, 이러한 안전설비는"이렇게 하면생존 가능성이 maximise된다"는 목표에 따라 설계된다. 소프트웨어 시스템도 마찬가지로 대규모 재해(데이터센터 붕괴 등)가 발생해도"얼마나 빨리 그리고 얼마나 완전하게객호에게서비스를 제공할 수 있게 회복하는가"를목표로 설계된다.

- **등장 배경 및 발전 과정**:
  1. **1990년대**: 인터넷 보급과 함께 웹 서비스의 중요성 증가, DR 개념 발전
  2. <strong>2001년 9/11</strong>: 미국 terrorist 공격으로 비즈니스 연속성 계획(BCP/DR)의 중요성 본격적 인식
  3. **2010년대**: 클라우드 컴퓨팅 확산으로 DR 구축 비용대폭적 감소
  4. **현재**: Ransomware 공격 증가에 따른 DR의 중요성 재인식, Cloud DR (DR-as-a-Service) 확산

- **📢 섹션 요약 비유**: DR 아키텍처는 <strong>'해상 Cruise선의 난파선 대피 프로그램'</strong>와 같다. cruise선이 침몰할 경우, 승객은 정해진 위치로 대피하고, 구명보트에 분산승좌하여 손실될 승객 수를 최소화하며, 구조 신호기를 작동시켜 구조대가 최대한 빨리 도착하도록 한다. 소프트웨어 시스템에서도 대규모 장애 시 데이터를 다른 곳에 복제하고, 미리정해진 절차에 따라 failover하여 피해량을최소화하는 것이 DR의 핵심이다.
...
재해 복구 (DR) 아키텍처의 핵심 원리는 **복잡성 분해**, **역할 분리**, <strong>품질 측정</strong>의 세 축으로 이해할 수 있다. 복잡한 문제를 관리 가능한 단위로 나누고, 각 역할의 책임을 명확히 하며, 결과를 정량적 지표로 평가하는 과정이 반복된다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 소프트웨어 공학 (Software 엔진ering) | 재해 복구 (DR) 아키텍처의 상위 학문 체계이며 품질·생산성 향상의 공통 목표를 공유한다 |
...
1. 재해 복구 (DR) 아키텍처은 레고 블록으로 성을 만들 때처럼, 규칙을 정하고 역할을 나누어 함께 작업하는 방법이에요.
