...
IT 관리, 시스템 설계 및 AI/데이터 사이언티스트를 위한 인공지능 전 영역 핵심 및 심화 키워드 800선입니다.
...
8. 지식 베이스 (Knowledge Base) / 추론 엔진 (Inference 엔진)
...
36. 특성 공학 (Feature 엔진ering) - 도메인 지식을 활용하여 모델 학습에 좋은 피처(Feature)를 추출/가공
...
143. 프롬프트 엔지니어링 (Prompt 엔진ering) - 모델의 최적 결과물을 이끌어 내기 위해 입력(명령/컨텍스트/예시) 텍스트를 최적화 설계하는 기술
...
420. AI 규제 신뢰성 ISO/IEC 42001 (AI Management System) 생명 주기 위험 통제 감사 투명 체계 가이드 프레임 평가 체제망 개념 파악 요약 등 (1~8장 완벽 실무자 800+ 통합 매핑망 적용 정리)


## 추가 학습 키워드 (Additional Study Keywords)

- Semantic Vs Instance Segmentation Fcn Unet Mask Rcnn

---
<strong>총정리 인공지능 / 알고리즘 키워드 : 총 800여 개 수록</strong> (+파생/분석 1,200개 커버 규모)
(머신러닝, 딥러닝 아키텍처는 물론 최근 폭발적인 LLM, RAG, 생성형 AI(Transformer, PEFT), 그리고 기반이 되는 확률/통계 선형대수학 수리 이론까지 IT 관리 수준의 방대한 지식 세트를 전면 확장하여 체계화하였습니다.)
