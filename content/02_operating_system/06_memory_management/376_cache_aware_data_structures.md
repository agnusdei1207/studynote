...
> 1. **본질**: 캐시 인식 데이터 구조(Cache-aware Data Structures)는 CPU 내부의 L1/L2 캐시 라인(Cache Line, 보통 64바이트)의 물리적 크기와 구조를 소프트웨어 개발자가 명시적으로 인지하고, <strong>메모리에 저장되는 데이터의 크기와 배열 방식을 캐시 라인에 정확히 맞춰 정렬(Alignment)</strong>하는 설계 기법이다.
> 2. **가치**: 캐시 미스(Cache Miss)를 극한으로 줄여 메모리 접근 속도를 수십 배 끌어올리며, 특히 멀티코어 환경에서 서로 다른 코어가 같은 캐시 라인을 수정하려다 발생하는 <strong>거짓 공유(False Sharing) 병목을 원천적으로 차단</strong>하여 시스템의 확장성(Scalability)을 보장한다.
...
<strong> 캐시 인식 해결책 (Cache-aware [Padding) ]</strong>
```c
struct {
    int A;
...
| <strong>SIMD 호환</strong> | 병렬 벡터 연산(AVX)을 태우기 까다로움 | 한 번에 x값 16개를 쓸어 담아 연산하는 **SIMD에 최적화** |
...
| 구분 | 내용 |
|:---|:---|
| <strong>거짓 공유(False Sharing) 박멸</strong>| 멀티 코어 간의 무의미한 캐시 일관성(MESI 프로토콜) 핑퐁을 없애, 코어 수에 정비례하는 선형적 스케일업 달성 |
| <strong>메모리 대역폭 절약</strong> | 캐시 라인 경계선에 걸쳐 램을 2번 읽어야 하는 지연을 없애 $O(1)$의 완벽한 1사이클 하드웨어 데이터 접근 보장 |
| <strong>SIMD 벡터화 극대화</strong> | 데이터를 연속적인 배열(SoA)로 팩킹하여 한 번의 CPU 명령어로 16개의 데이터를 동시에 덧셈하는 기계적 쾌감 실현 |
...
| 개념 | 연결 포인트 |
|:---|:---|
| 주소 공간 무작위 배치 (ASLR, Address Space Layout Randomization) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 메모리 보호 키 (Memory Protection Keys) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| NUMA (Non-Uniform Memory Access) 아키텍처와 메모리 할당 정책 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
...
1. 캐시 인식 데이터 구조 (Cache-aware Data Structures)은 컴퓨터가 메모리를 방처럼 나눠 쓰고 주소를 찾는 방법이에요.
2. 먼저 메모리 보호 키 (Memory Protection Keys)을 이해하면 캐시 인식 데이터 구조 (Cache-aware Data Structures)이 왜 필요한지 더 쉽게 보여요.
3. 그래서 캐시 인식 데이터 구조 (Cache-aware Data Structures)을 잘 알면 나중에 NUMA (Non-Uniform Memory Access) 아키텍처와 메모리 할당 정책도 훨씬 쉽게 배울 수 있어요.
