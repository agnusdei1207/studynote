...
| <strong>취소 점점</strong> | 커널이 자동 검사하는 지점 | pthread_cond_wait() 등 블로킹 API |
...
| 개념 | 연결 포인트 |
|:---|:---|
| 좀비 프로세스 (Zombie Process) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 고아 프로세스 (Orphan Process) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 취소 점 (Cancellation Point) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 스레드 로컬 저장소 (TLS, Thread-Local Storage) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
