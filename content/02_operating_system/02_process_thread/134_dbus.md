...
|:---|:---|:---|:---|:---|
| <strong>버스 이름 (Bus Name)</strong> | 버스에 연결된 프로세스의 식별자 | 잘 알려진 이름(Well-known Name)과 고유 이름(Unique Name)으로 구성 | DNS, Java RMI | 회사명 |
...
| <strong>RPC 의미론</strong> | 네이티브 지원 | 미지원 (데이터 교환만) | 애플리케이션 단에서 구현 |
| <strong>서비스 발견</strong> | 버스 이름 기반 자동 | 불가 (경로를 사전 알아야 함) | 불가 (소켓 경로 사전 필요) |
...
- <strong>정보보안 (IS, Information Security) 관점</strong>: D-Bus 버스 데몬은 메시지에 대한 접근 제어(ACL, Access Control List)를 수행한다. `/etc/dbus-1/system.conf` 설정 파일을 통해 특정 버스 이름에 대한 메서드 호출 권한을 사용자 그룹 단위로 제어할 수 있다. 예를 들어 `org.freedesktop.NetworkManager`의 설정 변경 메서드는 `root` 또는 `netdev` 그룹에만 허용하도록 정책을 구성할 수 있다.
...
<strong>실무 시나리오 1. systemd 서비스 상태 모니터링</strong>:
...
<strong>실무 시나리오 2. 버스 데몬 장애로 인한 전체 데스크탑 응답 불가</strong>:
...
|:---|:---|:---|:---|
| <strong>서비스 발견</strong> | 사전 지식 필요 | 버스 이름으로 자동 해결 | 마이크로서비스 동적 구성 가능 |
| **브로드캐스팅** | 수동 구현 | 시그널 네이티브 지원 | 이벤트 기반 아키텍처 실현 |
| <strong>지연 시간</strong> | ~1 us (직통) | ~10~50 us (데몬 경유) | 고성능 I/O에는 부적합 |
