...
### 실무 시나리오
1. <strong>데이터베이스 커넥션 풀</strong>: Pthread mutex + 조건 변수로 연결 획득/반환을 동기화. 풀이 비면 `pthread_cond_wait`으로 대기, 반환 시 `pthread_cond_signal`로 깨움.
2. <strong>병렬 과학 계산</strong>: `pthread_barrier_wait`로 각 계산 단계 완료를 동기화. 모든 스레드가 현재 단계를 마쳐야 다음 단계로 진행.

### 안티패턴
- <strong>mutex 초기화 누락</strong>: Pthreads는 명시적 초기화가 필요. `PTHREAD_MUTEX_INITIALIZER`나 `pthread_mutex_init()` 빠뜨리면 미정의 동작.
