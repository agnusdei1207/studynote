...
2. <strong>Windows 서비스 종료</strong>: 수동 리셋 Event로 모든 워커 스레드에 종료 신호를 동시 전파, WaitForMultipleObjects로 완료 확인.

### 안티패턴
- <strong>Critical Section 중에 대기 함수 호출</strong>: CS 보유 중 `WaitForSingleObject` 호출 -> 교착 상태. CS는 반드시 최소 범위로 유지.
