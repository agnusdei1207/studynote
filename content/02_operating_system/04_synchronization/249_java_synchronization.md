...
2. <strong>배치 처리 완료 대기</strong>: `CountDownLatch`로 N개 작업 스레드가 모두 완료될 때까지 메인 스레드를 대기시키는 패턴. 유한 버퍼 소비자의 일괄 처리 완료 확인에 활용.
...
- <strong>이중 락 확인(Double-Checked Locking) 미완성</strong>: `volatile` 없이 구현하면 JIT 최적화로 초기화 전 객체 참조 노출.
