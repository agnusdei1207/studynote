...
> 1. **본질**: 파일 지원 메모리(File-backed Memory)는 물리 램(RAM)에 올라온 특정 데이터 조각이 <strong>하드디스크의 실제 파일(예: <code>.exe</code>, <code>.so</code>, <code>.txt</code>) 내용과 1:1로 매핑(Mapping)되어, 확실한 원본(고향)을 두고 있는 축복받은 메모리 공간</strong>을 의미한다.
> 2. **가치**: 램이 꽉 차서 이 메모리를 쫓아내야(Eviction) 할 때, 내용이 바뀌지 않은(Clean) 코드 영역이라면 <strong>느린 스왑(Swap) 디스크에 쓸 필요 없이 램에서 그냥 삭제해 버리는(Drop) O(1) 초고속 처리가 가능</strong>하여 시스템의 램 회수 능력을 극대화한다.
> 3. **융합**: 운영체제의 파일 입출력 로직(`read/write`)을 가상 메모리의 페이징 시스템으로 덮어씌워버린 <strong>mmap(Memory-Mapped File)</strong> 시스템 콜과 완벽히 융합되어, 파일을 읽을 때 메모리 복사 오버헤드를 제로화하는 현대적 I/O 아키텍처를 완성했다.
...
- **등장 배경 및 I/O 철학의 진화**:
  1. <strong>과거의 파일 읽기</strong>: 파일을 읽으려면 `read()`를 호출해 디스크에서 커널로, 커널에서 유저 램으로 데이터를 2번씩 복사(Memcpy)하는 생노가다를 뛰었다.
...
파일 지원 메모리가 램에서 쫓겨날 때의 운명을 가르는 가장 중요한 상태 비트다. 페이지 테이블 엔트리(PTE) 안에 있는 <strong>M (Modify) 비트 / Dirty 비트</strong>가 이를 판별한다.

- <strong>Clean Page (수정 안 됨)</strong>: 디스크에서 퍼온 원본과 램의 데이터가 100% 똑같은 상태. (주로 실행 코드). 쫓아낼 때 그냥 **삭제(Discard)** 하면 끝이다. I/O 오버헤드 0.
- <strong>Dirty Page (수정됨)</strong>: 램으로 퍼온 뒤 CPU가 값을 바꿨다. (예: 워드에서 글씨를 침). 디스크 원본 파일과 내용이 달라진 상태. 쫓아낼 때 반드시 디스크 원본 파일에 <strong>동기화 기록(Write-back)</strong>을 한 뒤 지워야 한다. I/O 오버헤드가 크다.
...
| <strong>스왑 아웃 시</strong>| <strong>무조건 스왑 파티션</strong>에 눈물겹게 묻힘 | **스왑 안 감.** (Clean은 삭제, Dirty는 원본 파일에 씀) |
| <strong>초기화 과정</strong> | 해킹 방지 위해 0으로 덮음 (ZFOD) | 디스크 원본을 읽어와서 채움 |
...
놀랍게도 이건 램이 꽉 찬 게 아니다. <strong>OS가 남는 램을 모조리 긁어모아 '파일 지원 메모리(Page Cache)'로 써먹고 있는 훈장</strong>이다.
...
- 즉, <strong>파일 지원 메모리는 OS의 가장 거대하고 유연한 완충 지대(Shock Absorber)</strong>로서, 시스템의 체감 속도를 HDD 속도에서 RAM 속도로 끌어올려 주는 기적의 캐시다.
...
3. <strong>mmap과 sendfile의 융합 (Zero-Copy)</strong>:
   - Kafka는 파일 데이터 1GB를 <strong><code>mmap</code> (파일 지원 메모리)</strong>으로 램에 딱 1번 올린다(Page Cache).
...
| 구분 | 내용 |
|:---|:---|
| <strong>Zero-Copy 통신 지원</strong> | 유저 모드 메모리 복사본(익명)을 만들지 않고 커널 페이지 캐시(파일 메모리)를 하드웨어(랜카드)로 직통 매핑해 시스템 대역폭 100% 활용 |
| **코드 공유(Sharing) 극대화**| 수백 개의 크롬 탭이 `chrome.dll` 파일을 요구해도, 물리 램에는 딱 1장만 올려놓고 공유하여 램 낭비 원천 차단 |
| <strong>OOM(메모리 부족) 지연 방어</strong>| 램 회수 시 스왑(Swap) 파티션에 쓰기(Write)를 해야 하는 최악의 오버헤드를 건너뛰고 0초 만에 램 확보(Clean Drop)를 달성 |
...
| 개념 | 연결 포인트 |
|:---|:---|
| 스왑 공간 (Swap Space) / 베이킹 스토어 (Backing Store) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 익명 메모리 (Anonymous Memory) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 쓰기 시 복사 (COW, Copy-on-Write) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| vfork() | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 파일 지원 메모리 (File-backed Memory)은 컴퓨터가 메모리를 더 크게 보이게 하고 부족함을 숨기는 방법이에요.
2. 먼저 익명 메모리 (Anonymous Memory)을 이해하면 파일 지원 메모리 (File-backed Memory)이 왜 필요한지 더 쉽게 보여요.
3. 그래서 파일 지원 메모리 (File-backed Memory)을 잘 알면 나중에 쓰기 시 복사 (COW, Copy-on-Write)도 훨씬 쉽게 배울 수 있어요.
