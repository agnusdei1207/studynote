...
> 1. **본질**: 프레임 할당(Frame Allocation)은 다중 프로그래밍 환경에서 한정된 <strong>물리 메모리(Free Frames)라는 파이를 시스템에 띄워진 여러 프로세스들에게 '몇 개씩 쪼개어 나눠줄 것인가'를 결정</strong>하는 운영체제의 거시적 자원 분배 전략이다.
> 2. **가치**: 특정 프로세스에 프레임을 너무 적게 주면 매 클럭마다 페이지 폴트를 뿜어내는 스래싱(Thrashing)에 빠져 죽고, 너무 많이 주면 다른 프로세스가 굶어 죽으므로(Starvation), 전체 시스템의 <strong>동시 실행 효율(Degree of Multiprogramming)과 성능을 극대화할 황금 밸런스</strong>를 맞춘다.
> 3. **융합**: 이 할당량의 크기는 <strong>균등(Equal)/비례(Proportional) 할당</strong>이라는 정적 규칙에서 출발하여, 프로세스의 활동량(Page Fault Frequency)에 따라 프레임을 뺏고 더해주는 동적 <strong>워킹 셋(Working Set)</strong> 개념과 융합되어 현대 OS의 적응형 스케줄링으로 진화했다.
...
- **등장 배경 및 분배의 딜레마**:
  ...
  3. <strong>알고리즘의 고도화</strong>: 무식하게 1/N로 나누는 균등 할당에서 시작해, 덩치에 비례해서 주는 비례 할당을 거쳐, 런타임에 눈치껏 조절하는 동적 할당으로 뼈대가 굳어졌다.
...
"아무리 적게 줘도 이것보단 많이 줘야 프로그램이 터지지 않는다"는 절대 하한선이 있다. 이는 OS가 아닌 <strong>CPU 하드웨어의 명령어 구조(ISA)</strong>가 결정한다.
...
- 이 명령어를 한 번 치기 위해선 <strong>무조건 최소 4장의 프레임</strong>이 물리 램에 깔려 있어야 한다. OS가 3장만 할당해 주면 4번째 장을 가져올 때 1번 장을 쫓아내게 되어, 명령어가 영원히 완성되지 못하고 제자리를 맴도는 무한 페이지 폴트 재앙이 터진다.
...
1. <strong>균등 할당 (Equal Allocation)</strong>
...
|:---|:---|:---|
| <strong>고정 할당 + 지역 교체</strong> | 딱 100프레임만 주고, 넘치면 니들 안에서 알아서 쫓아내며 살아라. **타 앱에 피해를 주진 않지만 램 활용도가 썩어 들어감**. | ❌ 낡은 메인프레임 방식 |
| <strong>가변 할당 + 전역 교체</strong> | 일단 주고, 더 필요하면 램을 휩쓸며 남의 방을 막 뺏어와라! **전체 램 가동률 99%의 멱살 캐리 가능하지만 남의 렉을 유발**. | 🟢 리눅스, 윈도우의 абсолют 스탠다드 |
...
+----------+------------+------------+------------------------------+
```
**[매트릭스 해설]** 리눅스는 지독한 능력주의(전역 교체)를 택했다. 열심히 일하는 놈이 폴트를 쳐서 램을 요구하면, 가만히 쉬고 있는 남의 프레임을 가차 없이 빼앗아 일하는 놈에게 퍼준다. 이로 인해 프로세스별 할당량은 매초마다 고무줄처럼 변하며, 이는 자연스럽게 다음 장에서 배울 <strong>동적 할당(Working Set)</strong> 아키텍처로 이어지게 된다.
...
2. <strong>cgroups (Control Groups)의 구원</strong>:
   - 구글 엔지니어들은 이 야생의 전역 교체를 통제하기 위해, 리눅스 커널에 <strong>cgroups</strong>라는 철창을 만들었다.
...
| 구분 | 내용 |
|:---|:---|
| <strong>스래싱(Thrashing) 억제</strong> | 무한정 램을 뺏고 뺏기는 악순환의 고리를 끊고, 최소한의 작동 보장선(Minimum Frames)을 지켜내어 시스템 붕괴를 방어 |
| **공정성(Fairness)과 효율성의 타협**| 무식한 N분의 1 균등 할당을 버리고 동적 전역 교체를 채택하여, 노는 램을 1바이트도 남기지 않는 100% 가동률 달성 |
| <strong>컨테이너 생태계의 뼈대</strong> | 할당량을 엄격히 통제하는 cgroups와 결합하여, 남의 자원을 훔쳐 쓰지 못하게 막는 클라우드 샌드박싱의 인프라로 진화 |
...
| 개념 | 연결 포인트 |
|:---|:---|
| 페이지 교체 (Page Replacement)의 필요성 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 변경 비트 (Modify Bit / Dirty Bit) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 균등 할당 (Equal Allocation) vs 비례 할당 (Proportional Allocation) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 전역 교체 (Global Replacement) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 프레임 할당 (Frame Allocation) 알고리즘은 컴퓨터가 메모리를 더 크게 보이게 하고 부족함을 숨기는 방법이에요.
2. 먼저 변경 비트 (Modify Bit / Dirty Bit)을 이해하면 프레임 할당 (Frame Allocation) 알고리즘이 왜 필요한지 더 쉽게 보여요.
3. 그래서 프레임 할당 (Frame Allocation) 알고리즘을 잘 알면 나중에 균등 할당 (Equal Allocation) vs 비례 할당 (Proportional Allocation)도 훨씬 쉽게 배울 수 있어요.
