...
- **概念**: RISC는 컴퓨터 아키텍처의 '미니멀리즘(Minimalism)'이다. "곱하기, 나누기, 루프 돌기 같은 복잡한 일은 전부 소프트웨어(컴파일러)가 알아서 조립해라. 우리 하드웨어는 더하기, 빼기, 옮기기 같은 아주 단순한 동작만, 대신 **세상에서 제일 빠르게(1 Cycle 만에)** 처리해 주겠다"라는 철학이다.
- **💡 비유**: RISC는 '햄버거병'을 고친 '규격화된 패스트푸드 주방'과 같다. 손님이 "양파 빼고 소스 두 번 넣은 패티 세 장짜리 버거(CISC의 복잡한 명령어)"를 주문하면 주방장이 멘붕에 빠진다. 그래서 RISC 주방은 "우리는 치즈버거, 불고기버거 딱 2개만 팝니다. 무조건 1분 안에 나옵니다"라고 메뉴를 줄이고(Reduced) 규격화했다. 패티 세 장이 먹고 싶으면 손님(컴파일러)이 불고기버거 3개를 시키면 된다.
- **등장 배경**: 
