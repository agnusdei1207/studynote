...
IT 관리, 시스템 설계 및 클라우드/플랫폼 엔지니어를 위한 데브옵스, SRE, CI/CD, GitOps, 컨테이너 보안(DevSecOps) 및 옵저버빌리티(Observability) 전 영역 800대 핵심 키워드입니다.
...
28. 플랫폼 엔지니어링 (Platform 엔진ering) - 개발자의 인지 부하(Cognitive Load)를 줄이기 위해 전담 플랫폼 팀이 '내부 개발자 포털(IDP)'을 구축해 툴체인을 셀프 서비스로 제공하는 최신 DevOps 트렌드
...
121. SRE (Site Reliability 엔진ering) - 구글이 제안한 IT 운영 접근법. "SRE는 소프트웨어 엔지니어에게 운영 업무를 맡겼을 때 발생하는 일이다."
...
148. 카오스 엔지니어링 (Chaos 엔진ering) - 시스템이 평상시일 때 고의로 서버 종료, 네트워크 지연, CPU 폭주 등 '혼돈(장애)'을 주입하여, 이중화/서킷 브레이커 같은 시스템의 회복 탄력성(Resiliency) 메커니즘이 실제 위기 시 정상 동작하는지 선제적으로 실험하는 엔지니어링 (넷플릭스 카오스 몽키 기원)
...
259. 카오스 보안 엔지니어링 (Security Chaos 엔진ering) - 프로덕션 시스템에 방화벽 정책 삭제, IAM 권한 오류 등을 고의로 주입하여 보안 관제 시스템(SIEM)이 제대로 알람/차단하는지 테스트
...
## 6. 시험 빈출 요약 및 실무자 융합 논술 토픽 (120개 집중 요약)
...
371. (데브옵스/클라우드 실무자 필수 심화 주제 논술 키워드 통합 800+ 라우팅 확장)
... (아키텍처 확장 패턴 지속)
...
- Build Release Run
- Zero Trust Architecture ZTNA SASE NIST SP 800-207
- Confidential Computing TEE SGX TrustZone
- Software Supply Chain Security SBOM CycloneDX
- Secret Management Vault KMS Key Rotation
- Container Image Signing Cosign Notary Sigstore
- Runtime Security eBPF Falco Tetragon
- Policy as Code OPA Rego Kyverno
- Automated Penetration Testing DAST ZAP
- Static Application Security Testing SAST SonarQube
- Software Composition Analysis SCA Snyk
- Interactive Application Security Testing IAST
- Runtime Application Self-Protection RASP
- Security Gate CI CD Pipeline Quality Gate
...
- Chaos Engineering Security GameDay Fault Injection
- Cloud Security Posture Management CSPM
- Cloud Workload Protection Platform CWPP
- Cloud Native Application Protection Platform CNAPP
- Data Protection Encryption Tokenization Masking
- API Security OAuth2 JWT Gateway OWASP
- GitOps Security Sealed Secrets SOPS
- Infrastructure Security Scanning Checkov Trivy
- Security Observability SIEM SOAR Detection
- DevSecOps Maturity Model DSOMM BSIMM
- DevSecOps Master Summary Integration Map

---
<strong>총정리 DevOps / SRE 키워드 : 총 800+ 심화 요약 수록 (하위 파생 1,000+ 규모)</strong>
(애자일/DevOps 방법론, CI/CD, GitOps(ArgoCD)부터 컨테이너 쿠버네티스 오케스트레이션, MSA 설계 패턴(Saga/CQRS), SRE 옵저버빌리티 인프라 및 최신 DevSecOps 공급망 보안까지 전 영역 실무자/전문가 수준의 키워드를 집대성했습니다.)
