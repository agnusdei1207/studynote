...
> 2. **가치**: 사용자 가치와 가까운 지표를 알람, 오토스케일링, 품질 판단에 연결한다.
> 3. **판단 포인트**: 카디널리티와 의미 없는 지표 폭증을 막기 위해 목적 중심으로 설계해야 한다.
...
또한 이 주제는 Prometheus, Business KPI, HPA처럼 주변 개념과 강하게 연결된다. 실무 관점에서는 개별 정의보다도 이런 연결 구조를 설명해야 답안의 깊이가 생긴다.
...
1. 커스텀 메트릭 비즈니스 로직 프로메테우스 연동의 목표 지표가 명확한가?
...
학습 정리에서는 "도입"만 쓰지 말고, 커스텀 메트릭 비즈니스 로직 프로메테우스 연동이 어떤 상황에서는 채택되고 어떤 상황에서는 단계적으로 적용되어야 하는지를 비용, 복잡도, 보안, 운영 역량 기준으로 분리해 적는 것이 좋다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. 커스텀 메트릭 비즈니스 로직 프로메테우스 연동은 복잡한 일을 순서와 규칙으로 정리해서 실수하지 않게 도와주는 방법이에요.
2. Prometheus 같은 친구들과 같이 움직여야 더 잘 작동해요.
