...
> 1. **본질**: 데이터 파이프라인 내 비정형 데이터 OCR 및 LLM 문서 파싱 파이프라인 (ETL): 1. **본질**: 기업의 데이터 파이프라인(ETL)에서, 표나 텍스트가 제멋대로 흩어진 PDF, 스캔 본 이미지 등 '비정형 데이터'를 AI가 읽을 수 있는 정형화된 JSON이나 마크다…를 이해하는 핵심 개념으로, 데이터에서 패턴을 학습해 예측·생성·판단 보조로 연결해야 하는 문제를 설명하는 데 쓰인다.
