...
> 1. **본질**: 자율주행 강화학습 모방 학습 (Imitation Learning / Behavior Cloning): 1. **본질**: 강화학습(RL)은 환경과 부딪히며 시행착오로 배우는 방법이지만, 실제 자율주행차로 시행착오(사고)를 내면 치명적이므로, 사람이 운전하는 데이터(정답)를 보고 그대로 따…를 이해하는 핵심 개념으로, 데이터에서 패턴을 학습해 예측·생성·판단 보조로 연결해야 하는 문제를 설명하는 데 쓰인다.
